    ...
    <div className="app-container">
      ...
      <aside className="sidebar">
        ...
        <div className="model-status">
          <div className="model-dot"><div className="dot green" />Pipeline: Gemini 3.1 Flash Lite</div>
          <div className="model-dot"><div className="dot amber" />Supreme: Gemini 2.5 Flash</div>
        </div>
      </aside>
          ...
          <>
            <section className={`pipeline-viz ${loopActive ? 'loop-active' : ''}`}>
              ...
              <StepBox title="Predictor" active={currentStep === 'predictor'}
                ...
                status={status === 'processing' && currentStep === 'predictor' ? 'Thinking...' : 'Gemini 3.1 Lite'}
              ...
              <StepBox
                title={auditorSource === 'Local' ? 'Auditor' : 'Supreme Auditor'}
                active={currentStep === 'auditor'}
                icon={<CloudLightning size={20} color={auditorSource !== 'Local' ? "var(--accent)" : undefined} />}
                status={status === 'processing' && currentStep === 'auditor' ? 'Auditing...' : auditorSource === 'Local' ? 'Gemini 3.1 Lite' : 'Gemini 2.5 Flash'}
...
const WorkflowModal = ({ cv, onClose, accentColor }) => {
  const renderLog = (event, idx) => {
    ...
    switch (event.event) {
      ...
      case 'predictor_start': msg = 'Predictor (Gemini 3.1 Flash Lite) thinking...'; break;
      case 'predictor_end':
        msg = 'Draft generated by Predictor.';
        type = 'success';
        thoughts = event.data.thoughts;
        break;
      case 'audit_start':    msg = 'Auditor (Gemini 3.1 Flash Lite) checking for bias...'; break;
      case 'audit_end': {
